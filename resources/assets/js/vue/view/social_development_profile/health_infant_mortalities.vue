	...
	<div id="health_infant_mortalities" >
	 	<div class="card rounded-0 bg-light mt-1 shadow-sm">
			<div class="card-body pl-4 pt-3 pb-3">
				<h5 class="font-weight-bold">Infant Mortality: Leading Causes, Number and Rate per 100,000 Population Comparative Five Year Average</h5>
			</div>
		</div>
		<div class="row">
			<div class="col-md">
					...
					<tbody>
						...
						<template v-for="item in health_infant_mortalities">
							<tr>
								...
								<td><button class="btn btn-danger btn-sm" @click="deleteInfo(item.id)">remove</button></td>
							</tr>
						</template>
					</tbody>
				</table>
...
<script>
	export default{
		...
		methods: {
			...
			updateInfo: function(){
				var vm = this;
				axios.post('/social_development/health_infant_mortalities/update', {data: vm.health_infant_mortalities})
						.then(data => {
							vm.$toastr('success', 'Information is now updated!', 'Infant Mortality: Leading Causes, Number and Rate per 100,000 Population Comparative Five Year Average')
							vm.getInfo(vm.year);
						})
			...
			addInfo: function(){
				...
				axios.post('/social_development/health_infant_mortalities/create', vm.form)
						.then(data => {
							vm.$toastr('success', 'Information is now created!', 'Infant Mortality: Leading Causes, Number and Rate per 100,000 Population Comparative Five Year Average')
							vm.getInfo(vm.year);
						})
			},
			deleteInfo: function(id){
				var vm = this;
				axios.post('/social_development/health_infant_mortalities/delete', {id: id})
						.then(data => {
							vm.$toastr('success', 'Information is now removed!', 'Infant Mortality: Leading Causes, Number and Rate per 100,000 Population Comparative Five Year Average')
							vm.getInfo(vm.year);
						})
			}
		},
